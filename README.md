...
面试不再强制按 `一面 → 二面 → 终面` 推进。轮次名称可以直接填写（例如业务面、试讲、加试或 HR 面）；每次面试结论为“通过”时，HR 必须手动选择下一环节是“继续面试”还是“进入 Offer”。选择继续面试后可自行安排并命名下一轮，选择进入 Offer 后状态变为 `Offer待发`，之后依次为 `Offer已发 → Offer已接受 / Offer已拒绝`。Web UI 支持面试评价、改期和取消。飞书日历只创建或更新招聘共享日历中的会议日程，不添加参与人；候选人在安排、改期、取消和面试前约一小时收到邮件。
