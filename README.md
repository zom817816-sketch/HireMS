...
| `embedding_factory.py` | 嵌入模型工厂：openai 兼容 API / 本地轻量模型（见「使用本地轻量 Embedding 模型」） |
...
| 变量 | 说明 | 默认值 |
|------|------|--------|
| `EMBEDDING_PROVIDER` | 嵌入后端：`openai`（OpenAI 兼容 API）/ `local`（本地轻量模型，见下文） | `openai` |
...
**Q：`EMBEDDING_PROVIDER=local` 启动报缺少依赖 / ImportError？**
A：安装可选依赖：`pip install -r requirements-local.txt`（langchain-huggingface + sentence-transformers）。首次运行会自动从 HuggingFace 下载模型，之后离线可用。
